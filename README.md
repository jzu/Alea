...
`initAlea()` returns and emits the block number. `getAlea()` returns and emits the hash.

In light of issue #1,the bounded version of `getAlea()` has been removed. The caller will have to adjust to the desired values.

Proceed with caution. Use this for small stakes only. 
